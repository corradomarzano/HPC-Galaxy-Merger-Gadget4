...
1. **Preprocessing:** A Python-based extraction tool (`Data_Table_Acquirer.ipynb/.py`) converts the binary HDF5 snapshots (stored in the ```scripts/data_hdf5``` folder) into structured `.csv` datasets (saved in the ```scripts/data``` folder) for efficient handling.

    How to configure the script:
     - Assign the local directory path containing your Gadget4 snapshots to the ```path_in``` variable;
     - Set ```path_out``` to the directory path where the ```data``` folder (containing the processed CSV files) will be created;
     - Set the ```N``` variable to the total number of snapshots generated by your simulation.
  
    How to run the script:
     - If you want to use the Python ```.py``` file, run the following command:
       ```bash
       python scripts/Data_Table_Acquirer.py
       ```
     - If you want to use the ```.ipynb``` Jupyter Notebook, open the script and click the command *'Run all'*.
...
2. **Analysis:** Post-processing and Data Analysis are performed in `Data_Analysis.ipynb`. This includes:
   ...
   How to configure the script:
     - Assign the local directory path containing your Gadget4 snapshots (Default: ```data_hdf5```) to the ```path_hdf5``` variable;
     - Set ```path``` to the ```data``` folder (containing the processed CSV files) directory path;
     - Set the ```N``` variable to the total number of snapshots generated by your simulation.
  
   How to run the script: open the notebook and click the command *'Run all'*, or run the single cells after running the first setup cell.
...
- **Key Evolution Phases:**
  ...
  <figure>
  <p align="center">
    <img width="900" height="900" src="plots/DM.png" />
  </p>
  <p align="center">
    <img width="900" height="900" src="plots/matter.png" />
  </p>
  </p>
  <figcaption>
    <p>
      <i> 
        $\hspace{26 mm}$ Snapshot 1 $\hspace{46 mm}$ Snapshot 28 $\hspace{46 mm}$ Snapshot 56 <br>
      </i>
    </p>
  </figcaption>
  <p align="center">
...
- `scripts/`: Python tools for data processing and scientific plotting. Stores also the raw and refined data needed by the scripts.
  
  *Note*: The full Gadget4 simulation dataset is too large to be fully uploaded on Github. To keep this repository lightweight and accessible, I have included a **"Toy Dataset"** consisting of the three key snapshots described in the *Key Results* section: 0 (Initial State), 28 (Merging Phase) and 56 (Merger Remnant). These snapshots allow you to fully test the ```data_acquirer``` and ```analysis``` scripts, to recreate the plots previously shown. If you are interested in the complete raw ```.hdf5``` data, feel free to contact me via [mail](mailto:corrado.marzano1739@gmail.com).
